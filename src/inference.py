...
def generate_panorama_image_only(
    model,
    condition_image=None,
    num_inference_steps=50,
    height=512,
    ...
    """
    Generate a panorama using only image conditioning (no text prompts)
    ...
    Returns:
        list or torch.Tensor: List of 6 PIL images or tensor of shape [6, 3, H, W]
    """
    print(f"Generating panorama with {num_inference_steps} steps (image-only conditioning)...")
    ...
    empty_text_embeddings = model.encode_text([""] * 6, device)
